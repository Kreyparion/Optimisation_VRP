...
//    cout << "Data importation done in " << elapsed_import.count() << " s" << endl;
    ...
    cout << "Heuristic Algorithm Result: " << heuristic_solver_score << " in " << elapsed_heuristic_solver.count() << " s" << endl;

    // Solve the problem with the exact solver
    float exact_solver_score = exact_solver(config, verbose);
    auto end_exact_solver = chrono::high_resolution_clock::now();
    ...
    cout << "CPLEX  Solver  Result : " << CPlex_score << " in " << elapsed_CPlex.count() << " s" << endl;*/
